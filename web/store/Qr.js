import { action, observable } from 'mobx';
import axios from 'axios';
import QRCode from 'qrcode';

class AliceStore {
    stores = null;
    constructor(stores) {
        this.stores = stores;
    }

    @observable url = null;
    @observable list = [];
    @observable itemToConfirm = null;
    @observable getListStatus = 'init';


    @action
    getList() {
        this.itemToConfirm = null;
        const { utils } = this.stores;
        utils.sleep(this.getListStatus === 'init' ? 0: 500).then(_ => {
            this.getListStatus = 'fetching';
            axios
                .get(`${process.env.API_HOST}/api/v1/qr`)
                .then(res => {
                    this.list = res.data;
                    this.getListStatus = 'success';
                })
                .catch(e => {
                    this.getListStatus = 'error';
                    console.log(e);
                });
        })
    }

    @action
    sendConfirm(item) {
        this.itemToConfirm = item;
        this.generateQrCode(item)
            .then(qrCode => {
                const formConfig = {};
                const formData = new FormData();
                formData.append('id', item.id);
                formData.append('qrCode', qrCode.toString());

                this.sendConfirmStatus = 'fetching';
                axios
                    .put(`${process.env.API_HOST}/api/v1/qr`, formData, formConfig)
                    .then(res => {
                        // this.sendConfirmStatus = 'success';
                    })
                    .catch(e => {
                        // this.sendConfirmStatus = 'error';
                        console.log(e);
                    })
            });
    }

    @action
    generateQrCode(item) {
        return new Promise((resolve, reject) => {
            QRCode.toDataURL(`http://10.20.3.54:3500/api/v1/qr/verify/${item.id}`, { errorCorrectionLevel: 'H' })
                .then(url => resolve(url.toString()))
                .catch(e => reject(e));
        })
        
    }
}

export default AliceStore;

